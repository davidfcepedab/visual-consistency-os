import { z } from "zod";
import { MutationControlSchema, createTraceId, structuredError, } from "./contracts.js";
import { driveIdFrom, readFullLibrarySnapshot, recordDriveId, stringValue, } from "./library-maintenance-tools.js";
/**
...
export const ApplyLibraryReconciliationInputSchema = MutationControlSchema.extend({
    actions: z.array(LibraryMutationActionSchema).min(1).max(20).superRefine((actions, context) => {
        const seen = new Set();
...
const FORBIDDEN_FIELD_PATTERN = /identity|face|tattoo|ring|detail_lock|identity_master|publication_ready/i;
...
function findExactHumanDecision(snapshot, fileId, captureId) {
    const capture = snapshot.captures.find((record) => stringValue(record.capture_id) === captureId &&
 ...
 * call is made.
 */
export function planSafeLibraryMutations(snapshot, actions) {
    const plan = [];
    const rejections = [];
...
export function createLibraryMutationHandlers(io) {
    ...
    async function readSnapshot(traceId, expectedRevision) {
        if (cachedSnapshot &&
    ...
    return {
        async applyLibraryReconciliation(rawInput) {
            ...
            if (!parsed.success) {
                return structuredError("INVALID_ARGUMENT", "visual_apply_library_reconciliation input is invalid", traceId);
            }
            ...
            try {
                ...
                if (!input.dry_run) {
                    ...
                    if (replayCode !== "NOT_FOUND") {
                        return structuredError(replayCode === "INVALID_ARGUMENT" ? "INVALID_ARGUMENT" : "BACKEND_ERROR", "Visual library idempotency lookup failed", traceId, replayCode !== "INVALID_ARGUMENT");
                    }
                }
                const snapshot = await readSnapshot(traceId, input.expected_revision);
                if (input.expected_revision !== snapshot.revision) {
                    return structuredError("CONFLICT", `expected_revision does not match the current library revision (${snapshot.revision})`, traceId);
                }
                const { plan, rejections } = planSafeLibraryMutations(snapshot, input.actions);
                if (rejections.length > 0) {
                    return structuredError("INVALID_ARGUMENT", `Unsafe or unresolved action(s): ${rejections
                ...
                const actionable = plan.filter((mutation) => mutation.operation !== "NOOP_ALREADY_REGISTERED");
                ...
                if (!isRecord(written) || written.ok === false) {
                    const backendError = isRecord(written) && isRecord(written.error)
                        ? written.error
                        : undefined;
                    const code = backendError && backendError.code === "CONFLICT"
                        ? "CONFLICT"
                        : backendError && backendError.code === "INVALID_ARGUMENT"
                            ? "INVALID_ARGUMENT"
                            : "BACKEND_ERROR";
                    return structuredError(code, "Visual library reconciliation write failed", traceId, code === "BACKEND_ERROR");
                }
                return {
                    ...written,
                    ok: true,
                ...
                return structuredError("BACKEND_ERROR", "Visual library reconciliation failed", traceId, true);
            }
        },
    };
}
