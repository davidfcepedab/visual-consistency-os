...
export const ListBatchesByProjectInputSchema = z
    ...
    .refine(({ date_from, date_to }) => !date_from ||
    !date_to ||
...
function recordId(record, preferredField, fallback) {
    return stringValue(record[preferredField]) || fallback;
}
function unknownLineage(capture, missingFields) {
    return {
...
export function classifyOrphans(snapshot) {
    const batchIds = new Set(snapshot.batches.map((batch) => stringValue(batch.batch_id)).filter(Boolean));
    const requestIds = new Set(snapshot.requests
        .map((request) => stringValue(request.request_id))
        .filter(Boolean));
    const captureIds = new Set(snapshot.captures
    ...
    const classifyCaptureReferences = (records, category, recordType, idField) => {
        records.forEach((record, index) => {
            const captureId = stringValue(record.capture_id);
            if (captureId && captureIds.has(captureId))
                return;
            results.push({
                category,
    ...
    classifyCaptureReferences(snapshot.reviews, "ORPHAN_REVIEW", "REVIEW", "review_id");
    classifyCaptureReferences(snapshot.result_memory, "ORPHAN_RESULT_MEMORY", "RESULT_MEMORY", "result_id");
    classifyCaptureReferences(snapshot.assets, "ORPHAN_ASSET", "ASSET", "asset_id");
...
function paginate(items, input, revision) {
    let offset = 0;
    if (input.cursor) {
...
function normalizeSubjects(batch) {
...
function classifyHandlerError(error, traceId) {
    const message = error instanceof Error ? error.message : "";
    if (message === "Stale pagination cursor") {
        return structuredError("CONFLICT", "Pagination cursor does not match the current dataset revision", traceId);
    }
    if (message === "Invalid pagination cursor") {
        return structuredError("INVALID_CURSOR", "Pagination cursor is invalid", traceId);
    }
    return structuredError("BACKEND_ERROR", "Visual OS backend request failed", traceId, true);
}
export function createSafeReadHandlers(backend) {
    return {
        async getCapture(rawInput) {
            ...
            if (!parsed.success) {
                return structuredError("INVALID_ARGUMENT", "capture_id must be a valid exact identifier", traceId);
            }
            try {
                ...
                if (!isRecord(response)) {
                    return structuredError("BACKEND_PROTOCOL_ERROR", "Visual OS backend returned an invalid capture response", traceId);
                }
                if (response.ok === false) {
                    ...
                    if (backendError.code === "NOT_FOUND") {
                        return structuredError("NOT_FOUND", "Capture was not found", traceId);
                    }
                    return structuredError("BACKEND_ERROR", "Visual OS backend request failed", traceId, true);
                }
                ...
                if (stringValue(response.capture.capture_id) !== parsed.data.capture_id) {
                    return structuredError("BACKEND_PROTOCOL_ERROR", "Visual OS backend did not return the requested capture", traceId);
                }
                return {
                    ok: true,
                    trace_id: traceId,
        ...
        async detectOrphanCaptures(rawInput) {
            const parsed = DetectOrphanCapturesInputSchema.safeParse(rawInput);
            const traceId = createTraceId(parsed.success ? parsed.data.trace_id : undefined);
            if (!parsed.success) {
                return structuredError("INVALID_ARGUMENT", "Orphan detection pagination input is invalid", traceId);
            }
            try {
                ...
                if (!isRecord(response) ||
                    ...
                    return structuredError("BACKEND_PROTOCOL_ERROR", "Visual OS backend returned an invalid orphan snapshot", traceId);
                }
                const snapshot = response.snapshot;
                const page = paginate(classifyOrphans(snapshot), parsed.data, snapshot.revision);
                return {
                    ok: true,
                    trace_id: traceId,
        ...
        async listBatchesByProject(rawInput) {
            const parsed = ListBatchesByProjectInputSchema.safeParse(rawInput);
            const traceId = createTraceId(parsed.success ? parsed.data.trace_id : undefined);
            if (!parsed.success) {
                return structuredError("INVALID_ARGUMENT", "Batch project query is invalid", traceId);
            }
            try {
                ...
                if (!isRecord(response) ||
                    ...
                    return structuredError("BACKEND_PROTOCOL_ERROR", "Visual OS backend returned an invalid batch catalog", traceId);
                }
                ...
                const batches = [...uniqueBatches.values()]
                    ...
                    .filter((batch) => statusSet.size === 0 || statusSet.has(stringValue(batch.status)))
                    .filter((batch) => humanStatusSet.size === 0 ||
                    humanStatusSet.has(stringValue(batch.human_status)))
                    .filter((batch) => {
                    if (subjectSet.size === 0)
                        return true;
                    const availableSubjects = new Set(normalizeSubjects(batch));
                    return [...subjectSet].every((subject) => availableSubjects.has(subject));
                })
                    .filter((batch) => {
                    ...
                    .sort((left, right) => stringValue(left.batch_id).localeCompare(stringValue(right.batch_id)));
...
function isOrphanSnapshot(value) {
